...
public class ConfeccionarPlanillaActivity1a extends AppCompatActivity {
...
//        se reflejen en la aplicacion
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot){
                ...
                if(p.getDatosAptitud()!= null) {
                    spinner_aptitud.setSelection(p.getDatosAptitud().intValue());
                }
                if(p.getDatosObjetivo() != null) {
                    et_objetivo.setText(p.getDatosObjetivo());
                }
                if(p.getDatosModalidad() != null){
                    spinner_modalidad.setSelection(p.getDatosModalidad().intValue());
                }
    ...
    public void onActionRegistrar(View view) {
        ...
        mRef.child("datosAptitud").setValue(spinner_aptitud.getSelectedItemId());
        mRef.child("datosObjetivo").setValue(et_objetivo.getText().toString());
        mRef.child("datosModalidad").setValue(spinner_modalidad.getSelectedItemId() );
